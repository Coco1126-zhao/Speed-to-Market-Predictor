...
The feature attribution analysis also produces commercially useful findings about *why* certain projects succeed, findings that hold up to scrutiny as first-principles market insights rather than statistical artefacts.
...
`service_type_NRIS/ERIS` is the top classifier feature. Projects requesting both Network Resource Interconnection Service and Energy Resource Interconnection Service simultaneously are the highest-completion-rate applicants in the dataset. These projects need full network access rights, a signal that the developer has committed serious capital and requires firm transmission rather than energy-only delivery. It is a behavioural indicator of project seriousness that is observable on day one.
...
The technology chart on the right tells a story that is largely about *when* these technologies entered queues rather than their inherent viability. **Nuclear (70%) and Hydro (50%)** rank at the top because the projects in this dataset are almost entirely pre-2000 legacy plants, submitted in an era of low queue competition, utility-sponsored capital, and regulatory certainty. These are not signals about future viability; they are artefacts of a very different market environment. **Fossil and Gas (35-38%)** similarly benefit from decades of history: gas peakers and combined-cycle plants dominated queue submissions in the 2000s and early 2010s when the queue was far less congested, and many completed before the current backlog conditions developed.

By contrast, **Solar (3%), Wind (3%), Battery (0%), and Hybrid (2%)** rank at the bottom despite being the dominant technologies in today's energy transition. This is not because these technologies are less viable in principle; it is because they make up the overwhelming majority of post-2015 submissions, entering queues already overwhelmed by competition. A Solar project submitted in 2021 is competing against hundreds of similar projects for the same transmission nodes, the same study slots, and the same grid upgrade budgets. The model has correctly learned that technology type and vintage year interact: being a Solar project in 2022 is a very different proposition from being a Gas project in 2008.

The commercially actionable takeaway from the technology chart is not the ranking itself but the interaction with region and timing: a Solar project in SPP filed in 2019 has a meaningfully different viability profile than the same project filed in CAISO in 2022, and the model quantifies that difference.
...
### 5. The classifier beeswarm reveals three layered signals driving completion probability

The SHAP beeswarm for the classifier shows a richer picture than feature importance rankings alone convey.

**Capacity has a non-linear effect.** `log_capacity_mw` is the top beeswarm feature, but the direction flips with scale: small projects (blue, low feature value) cluster on the positive side, while very large projects (red) push the prediction downward. Small projects are often simpler to study and face fewer network upgrade requirements; very large projects attract more regulatory scrutiny and face higher financial hurdles to completion.

**Queue year is a strong negative signal for recent entrants.** `queue_year` shows that projects entering the queue in recent years have markedly lower predicted completion probability. This captures the post-2019 explosion in solar and battery submissions that has overwhelmed study capacity across most ISOs, making it structurally harder for any individual project to complete.

**Queue backlog adds directional but modest signal.** `log_queue_backlog` shows high backlog nudging completion probability slightly downward, consistent with the hypothesis that congested queues have higher withdrawal rates. The effect is real but smaller than regional identity or service type, suggesting that *which* ISO a project is in matters more than *how many* projects are currently ahead of it.
...
A large-capacity Fossil project in ISO-NE where `log_capacity_mw` is the dominant positive contributor. The `is_hybrid` flag partially offsets the positive signals, but the combination of scale, technology type, and regional market structure is enough for the classifier to assign high completion probability.

![SHAP Waterfall Fast Mover](outputs/shap_waterfall_fast_mover.png)

### High-risk project (low completion probability)

A project where very large capacity and high queue backlog both push the prediction strongly negative. The `service_type_NRIS/ERIS` feature further suppresses completion probability: in this case the hybrid service request reflects complexity rather than commitment. Every feature in this waterfall pulls the prediction below the model baseline.
...
The AUC-PR drop from val (0.384) to test (0.114) reflects the declining completion rate in recent cohorts, not model degradation. Projects entering queues in 2022-2024 have not had sufficient time to complete; the test set positive rate of 2.3% is a data maturity issue rather than a signal failure.
...
**Modelling subset:** 24,690 completed or withdrawn projects (active and suspended projects excluded from supervised targets)
...
**Correlation, not causation.** SHAP feature importance is associative. High importance of ISO region reflects correlated structural differences between markets (regulatory environment, grid topology, market design) rather than a single causal mechanism.
...
**Missing data.** Approximately 50% of projects lack a recorded completion or withdrawal date, limiting the regression training set to 1,722 rows. A survival model (Cox PH or accelerated failure time) would allow active projects to be included as censored observations and is a natural next step.
...
`features.py` does not need to be run directly. It is imported automatically by both `models.py` and `shap_analysis.py` as a shared feature engineering module.
...
Claude (Anthropic) was used as a coding collaborator to generate the Python code across `data_loader.py`, `features.py`, `models.py`, and `shap_analysis.py`, and to debug issues that came up during execution (index alignment errors in model evaluation, duplicate column handling in SHAP profiling). The problem framing, choice of data source, feature design decisions, and all findings interpretations are my own work.
